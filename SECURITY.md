...
- **Per-project forbidden paths and commands.** `/forbidden <path>` makes a path untouchable and the tools refuse it. Only you can lift it, per session, through an approval prompt. The model cannot lift it or work around it. Patterns are **relative to the project root**, so they do not extend into a folder added with `/include` — that is a different project with its own list. The built-in secret protection above is not root-scoped and covers every root. Forbidden *commands* match on word boundaries, so forbidding `rm` refuses `rm -rf` without refusing `npm run warm`.
